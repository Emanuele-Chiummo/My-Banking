(function () {
    ...
    // ---- Visibilità importi & patrimonio (toggle unico) ----
    (function () {
        ...
        function sync() {
            ...
            if (icon) {
                icon.classList.toggle('bi-eye', !masked);
                icon.classList.toggle('bi-eye-slash', masked);
            }
            btn.setAttribute('aria-pressed', String(masked));
            // sfoca tutti gli importi con .money
            document.querySelectorAll('.money').forEach(el => el.classList.toggle('masked', masked));
        }
